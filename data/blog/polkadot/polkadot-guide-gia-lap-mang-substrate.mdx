...
Trước khi đi sâu hơn ở các phần sau của bài viết. Bạn hãy xem qua trước bài viết về [Polkadot Guide: Chạy Substrate node trên máy tính của bạn](https://lowlevelers.com/blog/polkadot/polkadot-guide-chay-local-substrate-node)và chắc chắn rằng bạn đã có thể ít nhất chạy được một node đơn lẻ. Ở đây mình sẽ chạy node đầu tiên sử dụng một `account` đã được định sẵn là `alice`
